...
function buildHtml(hostname: string, scores: ScoreInput[]): string {
...
export async function generateAuditPdf(
  input: AuditPdfInput
): Promise<{ pdf: Buffer; hostname: string }> {
  const hostname = hostnameFromUrl(input.url);
  const html = buildHtml(hostname, input.scores);
  ...
  try {
    ...
    return { pdf: Buffer.from(pdf), hostname };
  } finally {
    await browser.close();
  }
}
